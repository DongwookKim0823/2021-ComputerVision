...
void m_BlobColoring(BYTE* CutImage, int height, int width)
{
	...
	float grayGap = 255.0f / (float)curColor;  //curColor는 덩어리 져있는 component 개수

	// 가장 면적이 넓은 영역을 찾아내기 위함 
	for (i = 1; i <= curColor; i++)   //curColor는 component 개수
	{
		if (BlobArea[i] >= BlobArea[Out_Area]) Out_Area = i; //i번 째 component사이즈를 여태 가장 큰 사이즈와 비교해서 같거나 크면 갱신
	}
	// CutImage 배열 클리어~
	for (k = 0; k < width * height; k++) CutImage[k] = 255; //CutImage는 main함수에서 받아온 output배열, 모두 하얗게 채움

	// coloring에 저장된 라벨링 결과중 (Out_Area에 저장된) 영역이 가장 큰 것만 CutImage에 저장
	for (k = 0; k < width * height; k++)
	{
		if (coloring[k] == Out_Area) CutImage[k] = 0;  // 가장 큰 것만 저장(size filtering) //coloring[k]에는 인덱스 정보만 들어가 있음
		//if (BlobArea[coloring[k]] > 500) CutImage[k] = 0;  // 특정 면적 이상 되는 영역만 출력
		//CutImage[k] = (unsigned char)(coloring[k] * grayGap);  //전경의 각각의 component들을 grayGap으로 채워나감, 모든 component 확인
	}
